...
public class WeiXinUtil {
    static{
        ...
        try {
            properties.load(WeiXinUtil.class.getClassLoader().getResourceAsStream("config.properties"));

            corpId=properties.getProperty("");
            secret=properties.getProperty("");
            addressBookSecret=properties.getProperty("");
     ...
     * @return
     */
    private String sendPostRequest(String url, Object param) {
   ...
   public void createDept(int deptId,int parentId,String name){
       ...
       if(!errorCode.equals(0)){
           throw new RuntimeException("创建部门异常，错误码:" + errorCode + " 错误原因：" + errorCodeMap.get(errorCode));
       }
   }
    ...
    public void deleteDept(String id){
        String url=MessageFormat.format(DELETE_DEPT_URL,getAccessToken(TOKEN_TYPE_ADDRESS_BOOK),id);
        String result=sendGetRequest(url);
        Map<String,Object> map=JSON.parseObject(result,HashMap.class);
        Integer errorcode= (Integer) map.get("errorcode");
        if(!errorcode.equals(0)){
            throw new RuntimeException("删除部门异常，错误码:" + errorcode + " 错误原因：" + errorCodeMap.get(errorcode));
